...
def _build_jobs() -> tuple[ScheduledJob, ...]:
    """Construct the job inventory with paths derived from the runtime repo root.

    Called lazily (not at import time) so that ``RENQUANT_REPO_ROOT`` and other
    environment overrides are respected at the point of use.
    """
    ...
    return (
        ScheduledJob(
            ...
            launchd_stdout_path=f"{root}/logs/retrain_panel/launchd_stdout.log",
            launchd_stderr_path=f"{root}/logs/retrain_panel/launchd_stderr.log",
        ),
        ScheduledJob(
            ...
            launchd_stdout_path=f"{root}/logs/retrain_patchtst/launchd_stdout.log",
            launchd_stderr_path=f"{root}/logs/retrain_patchtst/launchd_stderr.log",
        ),
        ScheduledJob(
            ...
            launchd_stdout_path=f"{root}/logs/retrain_alpha158_linear/launchd_stdout.log",
            launchd_stderr_path=f"{root}/logs/retrain_alpha158_linear/launchd_stderr.log",
        ),
        ScheduledJob(
        ...
        ScheduledJob(
            ...
            native_cutover_command=[
                ...
                "readonly-alpaca",
                "--strategy-dir",
                f"{root}/backtesting/renquant_104",
                "--runs-db",
                f"{root}/data/runs.alpaca.db",
            ...
            launchd_stdout_path=f"{root}/logs/daily_104/launchd_stdout.log",
            launchd_stderr_path=f"{root}/logs/daily_104/launchd_stderr.log",
        ...
        ScheduledJob(
            job_id="intraday_quote_logger",
            kind="ops",
            cadence="intraday_session",
            command=["renquant-orchestrator", "run-job", "intraday_quote_logger"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.intraday-quote-logger",
        ),
        ScheduledJob(
            job_id="intraday_pairing_logger",
            kind="ops",
            cadence="intraday_session",
            command=["renquant-orchestrator", "run-job", "intraday_pairing_logger"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.intraday-pairing-logger",
        ),
        ScheduledJob(
            job_id="entry_timing_shadow",
            kind="ops",
            cadence="intraday_session",
            command=["renquant-orchestrator", "run-job", "entry_timing_shadow"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.entry-timing-shadow",
        ),
        ScheduledJob(
            job_id="intraday_session_scheduler",
            kind="control",
            cadence="intraday_session",
            command=["renquant-orchestrator", "run-job", "intraday_session_scheduler"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.intraday-session-scheduler",
        ),
        ScheduledJob(
            job_id="realtime_data_plane",
            kind="ops",
            cadence="intraday_session",
            command=["renquant-orchestrator", "run-job", "realtime_data_plane"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.realtime-data-plane",
        ),
        ScheduledJob(
            job_id="shadow_realtime_serving",
            kind="inference",
            cadence="intraday_session",
            command=["renquant-orchestrator", "run-job", "shadow_realtime_serving"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.shadow-realtime-serving",
        ),
        ScheduledJob(
            job_id="scorer_identity_monitor",
            kind="ops",
            cadence="daily",
            command=["renquant-orchestrator", "run-job", "scorer_identity_monitor"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            umbrella_state_dependency="RenQuant model artifacts for identity comparison",
            launchd_label="com.renquant.scorer-identity-monitor",
        ),
        ScheduledJob(
            job_id="fallback_shadow_logger",
            kind="ops",
            cadence="daily",
            command=["renquant-orchestrator", "run-job", "fallback_shadow_logger"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
        ...
        ScheduledJob(
            job_id="parking_sleeve_shadow",
            kind="ops",
            cadence="daily",
            command=["renquant-orchestrator", "run-job", "parking_sleeve_shadow"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            launchd_label="com.renquant.parking-sleeve-shadow",
        ),
        ScheduledJob(
            job_id="readiness_monitor",
            kind="ops",
            cadence="daily",
            command=["renquant-orchestrator", "run-job", "readiness_monitor"],
            owner_repo="renquant-orchestrator",
            migration_state="native_multirepo",
            production_safe=True,
            umbrella_state_dependency="RenQuant data paths and runs.alpaca.db for readiness checks",
            launchd_label="com.renquant.readiness-monitor",
        ),
        ScheduledJob(

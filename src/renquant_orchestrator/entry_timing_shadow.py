...
Tick-feed contract (reuse — do NOT re-collect): this consumes the SAME
``intraday_ticks.jsonl`` structured feed produced by the decoupled intraday quote
logger (orchestrator #216) and read by the paired-IS harness (#215). It reuses that
feed's **session / causality / freshness rules** rather than re-collecting quotes:

  * **Session** — only ticks inside regular US-equity trading hours
    (weekday, [09:30, 16:00) ET, America/New_York) are considered; out-of-session
    rows are dropped, never re-collected. The §11b entry window is stricter still:
    no entry in the first ``entry_open_offset_min`` minutes after the open nor in
    the last ``entry_close_cutoff_min`` minutes before the close.
  * **Causality** — each policy is a pure function that walks the tick series in
    ...
    It NEVER looks forward for a more favorable price; a later favorable tick is not
    chosen. Running state (VWAP, opening range) is accumulated only from ticks at or
    before the tick being evaluated.
  * **Freshness** — a tick whose quote age (sample ``ts`` − exchange ``quote_ts``)
    exceeds ``quote_staleness_hard_sec`` (design §6 class-D hard-skip) is dropped;
    an unpriceable row (no ``mid``) is skipped, never imputed.

The policy set + its parameters are **PRE-REGISTERED** here as a frozen config
(:data:`DEFAULT_CONFIG`, fingerprinted onto every row) and in the progress doc, so
the future experiment (§9.4) knows exactly which frozen policy produced the corpus.
...
# future experiment (§9.4) can migrate cleanly.
SCHEMA_VERSION = "1"
STAGE = "renquant105-stage1-operations-only"
RECORD_KIND = "observe_only_entry_timing_shadow"

# Regular trading hours, US equities, America/New_York — mirrors the #216 tick
# feed's session rule (weekday + [09:30, 16:00) ET) so the two agree on what an
# in-session tick is.
...
# ---------------------------------------------------------------------------
# Pre-registered frozen policy config
# ---------------------------------------------------------------------------
@dataclass(frozen=True)
class EntryTimingConfig:
    """PRE-REGISTERED, frozen entry-timing policy parameters (design §5.3, §6, §11b).

    Every field is a policy knob fixed BEFORE data collection so the corpus is
    reproducible and the future experiment (§9.4) can attribute rows to an exact
    parameterization via :meth:`fingerprint`. Long/buy side only (the shorting
    mandate bar is very high; entry-timing pilots the long entry).
    """

    # §11b session-boundary entry window (minutes relative to the open/close).
    ...
    def fingerprint(self) -> str:
        """Stable short hash of the frozen params — pins the corpus to this config."""
        payload = {
            "entry_open_offset_min": self.entry_open_offset_min,
            "entry_close_cutoff_min": self.entry_close_cutoff_min,
            "quote_staleness_hard_sec": self.quote_staleness_hard_sec,
...
def preregistration_manifest(config: EntryTimingConfig = DEFAULT_CONFIG) -> dict[str, Any]:
    """The frozen PRE-REGISTRATION of the candidate policy set + params.

    A single canonical dict — the ordered policy set, each policy's frozen params,
    the config fingerprint, and the schema/stage — surfaced so the progress doc, the
    ``--print-preregistration`` CLI, and the future experiment (§9.4) all reference
    the exact same frozen definition. Emitting or asserting this NEVER collects data
    or places anything (observe-only)."""
    return {
...
@dataclass(frozen=True)
class Tick:
    """A normalized in-session timing quote (class-D). ``mid`` is the decision-time
    reference; ``tick_time`` is the exchange as-of (idempotency/causality axis)."""
...
class PolicyOutcome:
    ...
    censored (never-triggered) outcome. Carries NO fill-quality metric."""
...
# ---------------------------------------------------------------------------
# Time / quote helpers (mirror the #216 feed's session + priceability rules)
# ---------------------------------------------------------------------------
...
def market_phase(now: datetime) -> str:
    """``"open"`` / ``"before_open"`` / ``"closed"`` in ET — the #216 session rule."""
...
def _quote_age_sec(record: Mapping[str, Any]) -> float | None:
    """Age (seconds) between the sample time (``ts``) and the exchange quote time
    (``quote_ts``). ``None`` when either is absent (age unknown → not dropped)."""
    sample, quote_ts = record.get("ts"), record.get("quote_ts")
    if not sample or not quote_ts:
        return None
    try:
        return (_parse_dt(sample) - _parse_dt(quote_ts)).total_seconds()
...
def normalize_ticks(
    ...
    *,
    ...
    """Filter a ticker's raw feed rows to the causal, in-session, fresh, priceable
    series and sort it ascending by ``tick_time`` (the causality axis).

    Reuses the #216 rules: drops unpriceable rows (no mid), out-of-session rows
    (not RTH), and stale rows (quote age > ``quote_staleness_hard_sec``). Does NOT
    re-collect anything — it only consumes the feed.
    """
    ticks: list[Tick] = []
    for r in records:
        ...
        if not tick_time:
            continue
        try:
            when = _parse_dt(str(tick_time))
        except (ValueError, TypeError):
            continue
        if not is_market_hours(when):
            continue  # out-of-session — dropped, not re-collected
        age = _quote_age_sec(r)
        if age is not None and age > config.quote_staleness_hard_sec:
...
# ---------------------------------------------------------------------------
# Entry window (§11b) + eligibility
# ---------------------------------------------------------------------------
def _session_bounds(date: str, config: EntryTimingConfig) -> tuple[datetime, datetime]:
    """The §11b entry window [open+offset, close−cutoff) as tz-aware ET datetimes."""
    ...
    return first_eligible, last_eligible
...
def _is_eligible(
    ...
    *,
    ...
    last_eligible: datetime,
    conviction: datetime | None,
) -> bool:
    """A tick may host an ENTRY iff it is inside the §11b window and at/after the
    name's conviction time. (State-accumulating ticks before the window are still
    used for VWAP / opening range, but an entry is never placed on them.)"""
...
def policy_opening_range_breakout(
    ...
    *,
    ...
    The opening range is the max mid over ticks in [open, open + opening_range_minutes)
    — built only from ticks inside that early window (causal). The breakout is then the
    first eligible tick after the window whose mid exceeds that high."""
    if not ticks:
        return PolicyOutcome(POLICY_OPENING_RANGE_BREAKOUT, eligible=False, censored_reason="no_ticks")
    day = ticks[0].when.astimezone(ET).date()
    open_dt = datetime.combine(day, RTH_OPEN, tzinfo=ET)
    or_end = open_dt + timedelta(minutes=config.opening_range_minutes)
    or_high = max((t.mid for t in ticks if open_dt <= t.when < or_end), default=None)
    if or_high is None:
...
def policy_pullback_to_ref(
    ...
    *,
    ...
    """Enter on the first eligible tick that dips to/below a reference level.

    The reference is the next-open **batch reference** when supplied (design §9.2:
    session-T open the 104 batch would get), else the session's first tick mid (the
    arrival reference). The trigger is a dip of at least ``pullback_pct`` below it."""
    if not ticks:
        return PolicyOutcome(POLICY_PULLBACK_TO_REF, eligible=False, censored_reason="no_ticks")
    ref = batch_ref if batch_ref is not None else ticks[0].mid
    if ref is None:
        return PolicyOutcome(POLICY_PULLBACK_TO_REF, eligible=False, censored_reason="no_reference")
    threshold = float(ref) * (1.0 - config.pullback_pct)
    for t in ticks:
        if t.mid <= threshold and _is_eligible(
            t, first_eligible=first_eligible, last_eligible=last_eligible, conviction=conviction
        ):
            return PolicyOutcome(POLICY_PULLBACK_TO_REF, eligible=True, entry_tick=t)
    return PolicyOutcome(POLICY_PULLBACK_TO_REF, eligible=False, censored_reason="no_pullback")
...
def evaluate_name(
    ...
    *,
    ...
    normalized (causal, in-session, fresh) tick series. One row per policy."""
    first_eligible, last_eligible = _session_bounds(name.date, config)
    ...
    for policy in config.policies:
        ...
        rows.append(build_record(name=name, outcome=outcome, config=config, batch_ref=batch_ref))
    return rows
...
def evaluate_session(
    ...
    *,
    ...
    ``tick_records`` is the raw #216 feed (already limited to the session, or a
    superset — it is filtered per name). ``batch_refs`` optionally maps ticker →
    next-open batch reference price for ``pullback_to_ref``. Returns all rows;
    writes nothing (caller appends)."""
    ...
    for name in admitted:
        ...
        rows.extend(
            evaluate_name(
                name, ticks, config=config, batch_ref=batch_refs.get(name.ticker)
            )
        )
    return rows
...
def collect(
    *,
    ...
    return evaluate_session(admitted, records, config=config, batch_refs=batch_refs)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="print the frozen pre-registered policy set + params (JSON) and exit; observe-only",
    )
    ...
    parser.add_argument(
        "--batch-refs-json",
        default=None,
        help="JSON object {ticker: next_open_batch_reference_price} for pullback_to_ref",
    )
    parser.add_argument(
        "--out",

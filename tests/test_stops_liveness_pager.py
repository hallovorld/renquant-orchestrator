...
umbrella checkout: the wrapper's checker/ntfy/paths are env-injectable
(RENQUANT_STOPS_PAGER_*), a throwaway local HTTP server records "pages", and
the installer's launchctl is replaced by a recording stub.

What is NOT covered hermetically (and is the operator landing demo instead):
real ntfy.sh delivery, real launchd bootstrap, and the checker running against
the pinned runtime (verified manually 2026-07-11, exit 0 — see the progress
doc). Tests here pin the contract: plist shape, live-topic wiring, page/no-page
per checker exit class, delivery-failure exit codes, echo-first dry-run.
"""
...
def _run_wrapper(tmp_path: Path, *, checker_exit: int, checker_msg: str,
    ...
    env = {
        ...
        "RENQUANT_STOPS_PAGER_CHECKER": str(checker),
        "RENQUANT_STOPS_PAGER_SUBREPO_ROOT": str(tmp_path),
...
def test_plist_parses_and_schedules_the_wrapper():
    ...
    sessions), no RunAtLoad, logs under the umbrella logs tree."""
    ...
    assert "logs/stops_liveness" in plist["StandardOutPath"]
    assert "logs/stops_liveness" in plist["StandardErrorPath"]
...
def test_wrapper_defaults_match_the_plist_topic_and_pinned_runtime():
    content = WRAPPER.read_text(encoding="utf-8")
    # ad hoc invocation (no plist env) must page the same live topic
    assert f'NTFY_TOPIC:-{LIVE_OPS_TOPIC}}}' in content
    # the checker imports the PINNED runtime module, not an installed package
    assert "renquant-pipeline/src" in content
    assert "renquant-common/src" in content
    assert "check_software_stops_liveness.py" in content
...
def test_test_fire_emits_one_marked_page_and_exits_zero(tmp_path, ntfy_server):
    ...
    assert "15m" in page["body"] and "60m" in page["body"]

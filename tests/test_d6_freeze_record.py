...
def _make_db(path: Path) -> None:
    ...
    def add(date: str, tickers, *, mu=0.01, fwd_60d=0.02):
        for t in tickers:
            rows_score.append((f"run-{date}", date, t, mu, 0.1, "BULL_CALM"))
            rows_fwd.append((date, t, 100.0, 0.001, 0.002, 0.003, 0.004, fwd_60d))
...
def test_session_enumeration_matches_loader_rule(env):
    ...
    # fwd_1d sees clean + window + fwd1d-only dates; single-ticker and
    # null-mu dates are never sessions.
    assert fwd1["n_available"] == len(CLEAN_DATES + WINDOW_DATES + FWD1D_ONLY_DATES)
    assert fwd60["n_available"] == len(CLEAN_DATES + WINDOW_DATES)
    all_ids = (fwd1["tuning"]["ids"] + fwd1["evaluation"]["ids"]
               + fwd60["tuning"]["ids"] + fwd60["evaluation"]["ids"])
    assert SINGLE_TICKER_DATE not in all_ids
    assert NULL_MU_DATE not in all_ids
    # data cutoff = max kept session per horizon
    assert rec["data_cutoff"]["fwd_1d_max_session"] == max(
        CLEAN_DATES + FWD1D_ONLY_DATES)
...
def test_verify_catches_db_drift(env, capsys):
    ...
    conn.execute(
        "INSERT INTO score_distribution VALUES ('run-x','2026-06-11','AAA',0.01,0.1,'BULL_CALM')")
    conn.execute(
        "INSERT INTO score_distribution VALUES ('run-x2','2026-06-11','BBB',0.01,0.1,'BULL_CALM')")
    conn.execute(
        "INSERT INTO ticker_forward_returns VALUES ('2026-06-11','AAA',100,0.001,0,0,0,0.02)")
    conn.execute(
        "INSERT INTO ticker_forward_returns VALUES ('2026-06-11','BBB',100,0.001,0,0,0,0.02)")
    conn.commit()
    conn.close()

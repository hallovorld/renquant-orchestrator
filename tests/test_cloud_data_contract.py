...
def test_verify_staged_missing_feature_dirs_warns_not_fails(staged_dirs: dict) -> None:
    """earnings_surprise/news_sentiment_alpaca are warn-only, not fail-closed."""
    report = verify_staged(**staged_dirs)
    assert report.passed
    feature_checks = {c.name: c for c in report.checks if c.name.startswith("feature_dir:")}
    assert set(feature_checks) == {
        "feature_dir:earnings_surprise",
        "feature_dir:news_sentiment_alpaca",
    }
    assert all(not c.required for c in feature_checks.values())
    assert all(not c.exists for c in feature_checks.values())
    assert len(report.failed) == 0


def test_verify_staged_present_feature_dirs(staged_dirs: dict) -> None:
    for dir_name in ("earnings_surprise", "news_sentiment_alpaca"):
        d = staged_dirs["data_staging"] / dir_name
        d.mkdir(parents=True, exist_ok=True)
        (d / "AAPL.parquet").write_bytes(b"stub")
    report = verify_staged(**staged_dirs)
    assert report.passed
    feature_checks = {c.name: c for c in report.checks if c.name.startswith("feature_dir:")}
    assert all(c.exists for c in feature_checks.values())

...
| `../catchup_guard.sh` (shared, `ops/`) | boot catch-up (orch#1085; moved out of this directory 2026-08-30 when the dawn preflight and the drift scan gained the same guard): sourced by `run_batch_scores_export.sh` / `run_session_scheduler.sh` as `launchd_catchup_guard <job> <date> <now> <slot> session <guard_log> <outputs>...`; with `RunAtLoad=true` in their plists launchd also invokes the wrapper at every bootstrap, and the guard runs the job only on an NYSE session day, only between the slot and that session's ACTUAL local close (r2: `../catchup_cutoff.py`, early-close aware — a non-session date or a calendar failure REFUSES), only when today's output is missing (skips stamp `logs/rq105/catchup_guard_<job>_<date>.log`, never the evidence log). The wrappers export `CATCHUP_CUTOFF_HELPER` before sourcing it | at load |
| `../catchup_cutoff.py` (shared, `ops/`) | the guard's calendar answer (orch#1085 r2): for a local date prints the local `HHMM` of that NYSE session's actual close (rc 0), or refuses — rc 1 non-session (weekend/holiday), rc 2 bad date / calendar import or backend failure. Runs under the wrapper's pinned `PYTHONPATH`; imports `renquant_orchestrator.intraday_quote_logger.default_session_calendar`, the same primitive `rq105_liveness_check.py` and the scheduler gate resolve; no `sys.path` bootstrap, no fallback | per guard call (`session` mode only) |

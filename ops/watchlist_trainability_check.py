...
def _watchlist(path: Path, label: str) -> set[str]:
    ...
    if not isinstance(wl, list) or not wl:
        raise InputMissing(
            f"the {label} config at {path} has no non-empty 'watchlist' — "
            f"refusing to compare against an empty set")
    return {str(t).strip().upper() for t in wl if str(t).strip()}


def _declared(rq_root: Path) -> tuple[dict[str, str], Path]:
    """The newest expected_non_trainable declaration, and where it came from."""
...
def undeclared_untrainable(rq_root: Path | None = None) -> tuple[list[str], dict]:
    ...
    declared, decl_path = _declared(root)

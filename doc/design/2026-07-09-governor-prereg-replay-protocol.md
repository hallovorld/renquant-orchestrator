...
- **Two distinct quantities, two distinct inference units — do not conflate them:**
  ...
  - **(ii) h-day BLOCK-aggregated return endpoints** (both kinds used in this
    ...
    **Frozen dependence-robust inference for unit (ii)**: (1) PRIMARY —
    mean of the block series with a Newey-West (lag 1) standard error
    computed ON THE BLOCK SERIES, small-sample-corrected by using the
    t-distribution with `N_blocks − 1` degrees of freedom; (2) predeclared
    robustness check — a stationary block bootstrap on the block series
    (expected block length 2 blocks, 10,000 resamples, percentile CI); an
    enable-grade conclusion must hold under BOTH (1) and (2), else the
    verdict is NOT enable-grade (conservative conjunction). (3)
    **Effective-sample-size criterion (frozen)**: `ESS = N_blocks ×
    (1 − ρ̂₁)/(1 + ρ̂₁)`, where `ρ̂₁` is the lag-1 sample autocorrelation of
    the block series (clipped below at 0); enable-grade additionally
    requires **`N_blocks ≥ 8` AND `ESS ≥ 6`**. This is the unit and method
    for §2a's P2 endpoint and for any other h-day block-aggregated estimand
    in this protocol (superseding both the r4 per-block-NW/pooling scheme
    and the r5 plain-t-test claim).
...
- Reaching `N_blocks = 8` (the frozen minimum — the low end of defensible,
  not a fully-powered target, and still subject to the `ESS ≥ 6` check on
  the realized dependence) requires **160 valid paired sessions (~8
  months); ≈ 200 scheduled sessions under the 20% missingness bound.** A
  fully-powered target (~30 blocks) would need ~600 sessions — multi-year,
  impractical for a shadow gate meant to unblock a breadth-bound deployment
  problem now; hence the honest low-power framing and the live-shadow
  requirement in §1.2 rather than any pretense of high power.
...
- **Tier 1 — mechanical early-REJECT check only.** From valid paired session
  ...
  - **P2-kill (available once ≥ 3 complete blocks exist, i.e. from valid
    session 60)**: the PAIRED ARM-LEVEL NET realized 20d block return
    (S-0.5 − S-1.0, read off the two actual ledgers — actual fills,
    realized tax, actual costs; the P2 endpoint above), averaged across all
    complete blocks, is below **−300 bps per 20d block**, with **at least 3
    complete blocks**. Gross returns are never used for this rule; no
    cohort statistic feeds it.
...
- **Symmetric no-early-ENABLE (no optional stopping in either direction)**:
  ...
  fraction lift, or a +300 bps paired arm-level net block mean — authorizes
...
- **Tier 2 — confirmatory read, block-gated (replaces the fixed
  "+10 extension" rule)**: continue running both arms until **`N_blocks ≥ 8`
  complete non-overlapping 20d blocks** accumulate — 160 valid sessions
  (≈ 200 scheduled under the 20% missingness bound; the realized endpoint
  has no maturation lag). At that point compute the preregistered §1.2
  unit-(ii) dependence-robust test on the paired arm-level block series
  (NW-on-blocks small-sample CI AND stationary bootstrap, in conjunction,
  plus the `ESS ≥ 6` check): the P2 bar is (a) paired arm-level net block
  mean ≥ 0 AND (b) the one-sided test rejects "mean ≤ −50 bps/block" (the
  non-inferiority margin below). This is the earliest point at which a
  RECOMMEND-ENABLE or a final (non-kill) REJECT verdict may be issued; a
  verdict issued before Tier 2's block gate is not decision-grade
  regardless of what the point estimate shows. **If, at any scheduled review
  point, complete blocks < 8 or ESS < 6, the outcome is
  NO-ENABLE-BY-DEFAULT** with exactly one predeclared extension: keep
  running until both minima are met (no peeking-based stop, no parameter
  change). DSR is computed on this same block series once the minima are
  met, deflated for the 2 arms actually compared (S-0.5, S-1.0 — a small
  trials-correction, named explicitly since it's non-zero). **PBO does not
  apply to this comparison** (§1.2) — a single preregistered 2-arm paired
  design has no combinatorial train/test structure for CSCV to run over;
  PBO is not computed for estimand (A) or (B), and its absence is not a
  gap, it is the correct treatment of a 2-arm design per §1.2.
...
- **Verdict = recommendation memo** (`doc/research/`, in this orchestrator
  ...
  4. run-bundle fingerprints are clean on every counted session-pair, the
     ...
     drift occurred.
  Anything less: REJECT — or, where the only failure is complete blocks < 8
  or ESS < 6, the honestly-underpowered NO-ENABLE-BY-DEFAULT with its single
  predeclared extension (keep running until both minima are met; no other

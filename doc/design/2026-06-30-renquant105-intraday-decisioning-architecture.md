...
**Rollout discipline (operations-only):** **readonly** (decide + log, place nothing) for **K = 5 sessions** → **canary = 1–2 allowlisted names** live, purely to **accumulate real paired-session execution data** under clean ops (no-leak + idempotency + reconciliation green **and** the Tier-1 critical-reject safety guard clean) → **expand cautiously on continued clean ops**. **No paper-designed statistical threshold gates expansion or go-live.** The safety envelope (§10), the Tier-1 critical-reject HARD halt, and the operational invariants above are the **only** Stage-1 gates. K and the canary allowlist are **operational** parameters, not a statistical pre-registration.

Stage 1 ships on **operational correctness alone**; the execution-quality question is deferred to §9.4. The engineering scaffold **plus** a clean corpus of real paired-session execution data are the deliverables.
...
4. Rollout (§9.3): are the **operations-only** readonly `K=5` / cautious-canary defaults right? (The formal execution-quality gate — sample size, block length, and whether a 10-bps effect is even identifiable at this account scale — is **deferred** to a separate **simplified experiment-prereg PR** finalized against **real pilot data**, §9.4.)

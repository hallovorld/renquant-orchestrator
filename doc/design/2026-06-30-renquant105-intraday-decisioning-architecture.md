...
A censored **intraday** cell is imputed to `IS_cap_hi` (assume the missing intraday execution was as *bad* as the frozen cap) and a censored **batch** cell to `IS_cap_lo` (assume the missing batch execution was as *good* as the frozen cap). Both directions **enlarge** `Δ = IS_intraday − IS_batch`, so the imputed Δ is the **worst case for PASS**: if the gate still passes when every censored cell is pushed maximally against it, censoring cannot have manufactured the PASS.

**Gating statistic — worst-case bound (replaces the filled-only median):** complete both arms over the admitted set with the table's adversarial imputation, then apply the §9.2c / prereg-§5 one-sided **upper confidence bound of Δ** to that imputed-complete admitted set. **PASS requires the worst-case upper CB of Δ to remain BELOW the +10-bps inferiority margin** (CI excludes the margin, per §9.2c). This is the only IS gate; the filled-only ("complete-case") median is **not** a gate.

**Frozen caps (researcher DOF → pre-registered, never set from canary data):** `IS_cap_hi` and `IS_cap_lo` are pinned in the synthetic-baseline pre-registration artifact (new §7 there), computed from the **pre-canary calibration distribution** (§9.2c §3): `IS_cap_hi` = the **95th percentile** and `IS_cap_lo` = the **5th percentile** of the calibration-window realized 104 next-open IS — a deliberately conservative, frozen support proxy (the only IS distribution that exists strictly before canary). They are frozen with the rest of the artifact and stamped via `synthetic_baseline_prereg_sha`.

**Sensitivity / tipping point (reported every session-window, must agree):** report (i) the **complete-case** Δ (drop censored — the optimistic naive readout), (ii) the **worst-case-bound** Δ (the gate), and (iii) the **censoring fraction by cause**. A **complete-case PASS with a worst-case FAIL is exactly the cherry-pick the gate exists to catch → NOT a PASS** (the gate is the worst-case bound; complete-case is descriptive only).
...
*Codex's additional round-4 note (no-fill / no-trigger pairs are not missing-at-random; counts alone are insufficient if a filled-only IS median drives promotion — wants an intent-to-treat outcome or a conservative penalty/sensitivity bound) was acknowledged-but-deferred in r5. It is **now RESOLVED in r6** — see §9.2d (ITT estimand + worst-case/Manski bound + frozen `IS_cap_hi` / `IS_cap_lo` caps + `c_max` evaluability precondition) and the §20 map below. It is no longer deferred.*
...
|---|---|---|---|
| 1 | No-fill / no-trigger pairs are **not missing-at-random**; reporting censoring **counts** is not enough if a **filled-only IS median** drives promotion — the worst executions can disappear. Define an **intent-to-treat operational outcome OR a conservative penalty/sensitivity bound** for no-fill pairs. | **Resolved** — IS estimand is now **ITT over the admitted pre-treatment set** (no admitted pair dropped); the gate is a **worst-case (Manski-style) bound** — censored *intraday* cells imputed to a frozen adversarial cap `IS_cap_hi`, censored *batch* cells to a frozen favorable cap `IS_cap_lo`, so Δ is maximised against PASS; **PASS = worst-case upper CB of Δ below +10 bps** (not a filled-only median). Complete-case Δ is reported alongside and must agree (complete-case-PASS / worst-case-FAIL → NOT a PASS). A frozen **`c_max`** makes the gate non-evaluable (operations-only) above it. Caps + `c_max` are pre-registered (artifact §7), frozen from the **pre-canary** calibration distribution — never canary data. | §9.1, §9.2d, §9.3, prereg §7 |

...
- **Uncertainty-band method + overlap PASS rule (frozen resampling DOF)** — a **session-level block bootstrap** (resample sessions with replacement, keeping intra-session fills together; `B = 10000`; fixed `rng_seed = 20260630`); on each replication `(k_spread, k_auction)` are **re-fit by the full constrained-Huber procedure**, so parameter + residual uncertainty come from one consistent resampling (not an assumed Gaussian). The **one-sided upper 95% CB** of the **matched-pair median IS difference** Δ (intraday − synthetic batch) = the 95th pct of the bootstrap `{Δ*}`. **PASS requires that upper CB to lie BELOW the +10-bps inferiority margin — i.e. the CI of the difference must EXCLUDE the margin, not merely the point estimate.** If +10 bps **overlaps** the interval, the result is "not distinguishable" → **NOT a PASS** (prereg §5).

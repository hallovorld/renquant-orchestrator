...
DATE: 2026-06-30
REVISION: r4 (2026-06-30) — addresses the Codex round-3 review of `320b4fc` (3 correctness fixes: split the economic target from gross submission-attempt quantity invariants so cancel/retry can't violate the accounting, make the Stage-1 execution-quality A/B operationally observable via a real intraday fill vs a synthetic quote-based batch fill model, and define the §9 metric denominators precisely with a min-count/confidence reject gate). r3 addressed the round-2 review of `268c0af`; r2 the round-1 review of `c976ed8`. Point-by-point maps in §16 (round 1), §17 (round 2), §18 (round 3).
...
Stage 1 commits to **(a)**; (b)/(c) are noted alternatives. The synthetic nature of the batch arm is stated plainly so the gate is read as *real intraday execution vs a modeled batch baseline*, not as two measured fills.
...
- **Execution-quality (IS) acceptance:** **real intraday-path median IS not worse than the synthetic batch-path median IS (§9.2b) by more than 10 bps**, over the matched admitted-pair set, with the synthetic model's uncertainty band reported alongside the point estimate.
...
- **Reject gate — minimum-count / confidence rule (replaces the bare "≤ 2%"):** a bare 2% threshold is uninformative at canary N (with `N = 20`, one reject already reads 5%, and zero rejects barely constrains a true 2% rate). So the reject gate may **fail only** when (i) attempted submissions reach a minimum `M_reject = 50`, AND (ii) the **lower bound of a one-sided 95% binomial (Clopper–Pearson) confidence interval on the observed reject rate exceeds the 2% ceiling** (i.e. ≥ 95% confident the true reject rate is above 2%). Below `M_reject` the reject readout is **informational only** and cannot block or pass expansion. This prevents a single noisy reject at small N from blocking and prevents zero-rejects-at-small-N from being read as proof of a low rate.
- **Rollout discipline (concrete):** **readonly** (decide + log, place nothing) for **K = 5 sessions** → **canary** = **1–2 allowlisted names** live → **expand only after N ≥ 20 matched admitted pairs** with clean ops (no-leak + idempotency + reconciliation all green). K, N, the 10-bps IS tolerance, the denominators (above), and the reject-gate parameters (`M_reject = 50` minimum attempted submissions + one-sided 95% binomial bound vs the 2% ceiling) are **the pre-registration**; changing them after canary data is observed invalidates the run. (If preferred, these can live in a separate `preregistration.md` artifact that must merge *before* canary starts — open question §15.4.)

...
**Gate-input census — REQUIRED before the replay test has any enforcement surface (amendment A5.1, 2026-07-02, §23).** The no-leak proof above can only check inputs someone actually classified into A/B/C/D — and the gate-stack has non-obvious temporal inputs this RFC does not yet name a class for: the **regime detector**'s daily-bar state (is a same-day regime read class A, frozen with the signal, or class B, snapshotted at the first eligible tick? — ambiguous as written above), the **earnings-blackout calendar**, and **wash-sale `last_sell_dates`** — which an intraday *sell* mutates **mid-session**, making it a class-C-like input with a subtlety the batch (104) system never had to handle (this is the same bug family as this repo's own STATE-EXT-SELL incident: a stamped-vs-actual state mismatch silently mis-gating a decision — get the classification of this one right). Without an exhaustive, closed-world enumeration, "hard-fail on partial-bar-into-A/B" (above) is aspirational, not enforced: an input nobody classified cannot be checked.

**The §8 pipeline slice's deliverables therefore include a machine-enforced, closed-world gate-input census artifact.** Every gate input the census tracks must record, at minimum:

| Field | Meaning |
|---|---|
| owner | which repo/component is responsible for producing/maintaining this input |
| temporal class | A / B / C / D per this RFC's own four-class framework |
| source timestamp | when the value was actually produced |
| availability timestamp | when the value became knowable to the decision (may lag source timestamp) |
| mutation semantics | can this value change mid-session after it was first read (e.g. wash-sale `last_sell_dates`, mutated by an intraday sell — the STATE-EXT-SELL bug family)? if so, how, and what re-read/consistency contract applies |
| fail-closed behavior | what happens if THIS specific input is stale, missing, or internally inconsistent when a tick reads it |

**The §6 replay test's own acceptance criteria must include, as a MECHANICAL CI requirement (not merely descriptive documentation): every input actually read by the gate stack during a replayed tick appears in the census; an input read by the gate stack but absent from the census is itself a test failure, on the same footing as a class-A/B leak.** This artifact and its wiring into the replay test do not exist yet; they are a named, required deliverable of the pipeline-repo build (§8), not optional polish.
...
| Entry order type (amendment A5.2, 2026-07-02, §23) | **pre-declared for all of Stage 1** — market vs. marketable-limit at a pre-declared NBBO±x bps — never left free per-order | order type dominates implementation-shortfall (IS) measurement more than any other envelope parameter; a Stage-1 pilot corpus mixing order types across entries would contaminate the future §9.4 experiment's data with an unrecorded confound |
...
## 23. Amendment integration map — A5.1-A5.3 measurement-integrity pins (2026-07-02)
...
Stacked after §22 (amendment A2): this section is numbered r14/§23, not a second r13/§22, since A2 and A5 are two distinct, sequentially-merged amendments from the same PR #223 review touching disjoint parts of §7/§10/§11 — see the REVISION header for the stacking rationale.

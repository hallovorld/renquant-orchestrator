...
2026-06-27. Part of the renquant105 suite. **Only entered if M2 passed AND DSR>0 still
holds on the shadow-period realized returns.** Given the feasibility prior, **this
milestone may never be reached** — and that is the correct outcome if the edge isn't there.
...
- F3.4 **SIP feed decision** ($99/mo Algo Trader Plus) for NBBO-accurate live fills — the
  IEX adverse-selection penalty is a meaningful fraction of the entire net edge.
...
| State | Condition |
|---|---|
| **Go-live precondition** | M2 green + DSR>0 holds on shadow-period realized |
| **On track** | live 21d Sharpe within **±0.5** of shadow-period Sharpe |
| **SCALE-UP (+1 gross step)** | 252d-equiv net Sharpe ≥ **1.0** AND max-DD shallower than **−10%** AND killed-winner ≤ 15% |
| **HOLD / de-risk** | dd −12..−15% or live Sharpe < 0.5 → freeze sizing |
| **KILL (hard, fail-closed)** | dd < **−20%** · single-session loss < **−5%** (halt buys, exits only) · OOS IC ≤ 0 ×10 runs · calibration slope ≤ 0 (21 runs) · live-shadow rank-corr ρ < 0.2 (5 runs) → revert to last-known-good champion |

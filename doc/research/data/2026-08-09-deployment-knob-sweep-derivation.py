...
def _runs(con, k=6):
    ...
    df = pd.read_sql(
        ...
        "  (select count(*) from candidate_scores cs where cs.run_id = pr.run_id "
        "   and cs.mu is not null and cs.sigma is not null and cs.sigma > 0) as n "
...
def sweep_run(con, run_id, run_date):
    ...
    n = len(cs)
    if n == 0:
        # zero scored candidates with finite mu/sigma -- the orch#931 serving
        # drift (sigma unstamped on live rows from 2026-05-12) makes recent
        # runs unusable for this sweep; skip and COUNT rather than crash
        return {"run_id": run_id, "run_date": run_date, "n_names": 0,
                "skipped": "no candidates with finite mu/sigma (orch#931)"}
    ...
    # (D) constructed lambda-sensitivity case (orch#945 review P0): a
    # HALF floor (min_invested=0.5) with a non-binding turnover cap. Here
    # lambda>0 SHOULD pull deployment above the floor if the term has any
    # force on real inputs; deployment pinned at the floor across every
    # lambda is the honest "lambda weak on these inputs" reading, and a
    # rising profile falsifies it.
    result["lambda_sensitivity_half_floor"] = [
        _solve(w_cur, n, tickers, mu, sigma, lam,
               0.5, NON_BINDING_TURNOVER)
        for lam in LAMBDAS
    ]
    return result

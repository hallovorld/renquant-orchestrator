> **PROVENANCE / DURABILITY RECOMMIT — added 2026-07-03 (S-REL audit item V6, step 1).**
> **Everything below this block is the original artifact, recommitted UNCHANGED in substance.**
>
> - **Origin:** PR **#199** "research(renquant105): Phase -1 cheap feasibility — measured σ_oc +
>   STOP/GO", branch `research/renquant105-phase-minus-1`, commit `dcd18b98`, opened 2026-06-27,
>   **CLOSED UNMERGED** 2026-06-27T20:03Z. Closure reason (recorded on the PR): renquant105 was
>   repointed away from the intraday design, so the question was superseded — the measurement
>   itself was never disputed; the branch was preserved.
> - **Why recommitted (S-REL V6 durability rationale):** merged plans on main actively rely on
>   this measurement — the H2 execution roadmap's non-goals, the unified 107 master plan's L3 row
>   ("phase −1 NO-GO not re-litigated"), and design-review amendment A4.2 (Stage-2 estimand pinned
>   to the conditional timing residual because of it) — while the memo, script, and tests lived
>   only on a closed-unmerged branch. A standing verdict's evidence must be on main
>   (`doc/design/2026-07-03-s-rel-experiment-reliability.md`, V6 step 1). Archival recommit only:
>   no re-litigation, no recomputation.
> - **Standing verdict vs the in-document verdict:** §7 below records **GO to M0** — the exact
>   application of the now-superseded intraday-105 pre-registered STOP/GO table, which did not
>   gate on net-edge. The STANDING verdict on main is the §6 finding promoted to operative:
>   **soft NO-GO on intraday open→close DIRECTIONAL alpha** — net edge **−6.4 bps @ IC 0.03 /
>   −3.4 bps @ IC 0.05** against the conservative 11 bps round-trip cost (equivalently, breakeven
>   needs σ_oc = cost/IC ≈ **367 / 220 bps** at IC 0.03 / 0.05, vs measured ≈152 bps std,
>   114–115 bps robust) → pivot to the execution-timing residual (A4.2, master-plan L3).
> - **Verification status: PROVISIONAL.** This recommit is durability only (V6 step 1); the full
>   adversarial recompute is a separate, later step (V6 step 2) and was deliberately NOT performed
>   here.
> - **Reopening conditions (standing, recorded):** measured cost below breakeven, or evidenced IC
>   far above the 0.03–0.05 band — either one arrives as a NEW frozen prereg, never a re-pitch.
> - **Evidence note:** no standalone evidence JSON was ever committed on the origin branch (the
>   script's `--json` output was not persisted); the measured evidence is the tables embedded in
>   §§2–7 below. Recovered set = this memo + the original progress record
>   (`doc/progress/2026-06-27-renquant105-phase-minus-1.md`) + the script + its 18 network-free
>   tests — all byte-identical to `dcd18b98` except this header and the one-line pointer on the
>   progress record.

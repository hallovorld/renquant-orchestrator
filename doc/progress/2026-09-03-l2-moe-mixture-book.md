...
EVIDENCE:  artifact:      `logs/l2_paper_bandit/l2_moe_mixture.jsonl` (written by the scheduled job once `-run` carries this merge)
           ...
           existing data: dry run over the real DBs 2026-09-03 07:10 PDT: 96 rows replayed, champion weight 0.504974 [VERIFIED — module stdout]; `tests/test_l2_paper_bandit.py` 13 passed (7 existing + 6 new: first-day/rule-2 weights, compounding + champion/best-arm tracking, missing intermediate mark ⇒ 0 contribution AND catch-up excluded, catch-up magnitude never enters the mixture nor the comparators, regret under one set of admissible observations, an intermediate mark between calendar dates is not a priced step, deterministic rewrite + CLI payload, verified log untouched) [VERIFIED — pytest 2026-09-03 at the reviewed head]

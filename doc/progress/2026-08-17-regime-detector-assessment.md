...
WHAT:      Commit `doc/research/2026-08-17-regime-detector-assessment.md`: a measured
           ...
           plane; P4 flicker; P5 +22td BEAR exit overhang; P7 bull-side split has no
           validated conditional signal in the served artifact's sanity ICs — its
           η²-vs-one-vol-threshold comparison is EXPLORATORY only, see memo E2), what
           is GOOD (BEAR entry: zero misses, mean +8td),
...
REVIEW FIX (Codex round 3, 2026-08-17): the REPLICATION script's phase-A consumers are
           now guarded too — the P6 split skips when part 2 ran without the corpus, and
           the manifest records the corpus identity pin (sha256 recorded at the
           corpus-present full run) instead of hashing absent files — so the reviewer's
           clean-checkout command runs end-to-end (verified: exit 0 in a corpus-absent
           worktree). Path normalization moved INTO the script (`rel()` emits
           `<repo>:<relpath>`), so the committed JSONs regenerate from the committed
           code; committed manifest refreshed from the corpus-absent run (phase-A
           entries in pin form), replication JSON regenerated byte-identical. Two new
           smoke tests pin the guard ordering and the no-absolute-paths invariant.
           This doc's P7 wording and TESTS list de-escalated: the η² comparison and
           BEAR IC split are exploratory (memo E1/E2), not decision-bearing evidence.

TESTS:     Smoke tests (now 5 after round 3): 5 passed in-session (Hurst-null exercised
           against a local umbrella kernel copy; env-skips on checkouts without one).
           Round-3 clean-worktree re-run of the replication script WITHOUT the phase-A
           corpus: exit 0; replication JSON regenerated byte-identical; manifest
           phase-A entries in identity-pin form (present_at_run=false). Derivation
           scripts previously re-run end-to-end in-session (exit 0); regenerated
           ...
           The BEAR IC split and the η² threshold comparison are EXPLORATORY (local
           corpus only; memo E1/E2) — not part of these checks and not decision-
           bearing. No repo test suite touched (docs + research-data artifacts only).

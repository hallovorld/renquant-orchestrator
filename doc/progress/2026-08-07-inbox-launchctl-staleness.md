...
STATUS:   READY FOR REVIEW. 6 new tests; inbox suite 33 passed
          `[VERIFIED — python3 -m pytest tests/test_agent_inbox.py -q]`. Full
          suite 6122 passed / 2 skipped / 3 failed
          `[VERIFIED — python3 -m pytest tests/ -q]`; all three failures
...
```
          ...
          `[VERIFIED — 2026-08-07]`: `rq104-risk-budget` sits at exit 1
          (documented CRITICAL, >=100% of a budget) with `newest log
          2026-08-01`. Re-running the module today gives
          `book_beta consumption=145.4% CRITICAL` but
          `per_name_concentration=78.5% OK` — the retained code no longer
...
best-known?:   yes for the limitation. **No** for coverage — 3 of 16 rows
               resolve a date `[VERIFIED — read_launchd_exits(), 2026-08-07]`;
               the rest write under names this lookup does not reach, and
               `None` is labelled "undiscoverable", never "no logs".
scope:         one helper + the render note; no classification logic changed.

Two log layouts are supported because both are in use
`[VERIFIED — `logs/` inspection 2026-08-07]`:
...
2. **That the 13 `None` rows have no logs.** They have no log this lookup
   finds. The distinction is the point.

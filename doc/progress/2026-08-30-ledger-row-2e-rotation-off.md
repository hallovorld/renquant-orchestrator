...
STATUS: ledger-only PR, row-2a/2b/2c/2d precedent: the authority row lands on
orchestrator `main` BEFORE the config PR merges. Row 2e is **evidence-backed**;
its first-hand, change-specific operator confirmation slot is **PENDING** —
the operator may confirm or veto. Unlike row 2d (blanket directive, package
parked), the 2026-08-30 directive explicitly delegates direction to the agent,
so the package proceeds with a single-key revert as the veto path.
...
oscillator forecast ×12 with zero modelled cost, and (c) produced the live
...
- Logic forensics: rotation `net_adv` = per-ticker 5d ER ×12 to 60d
...
- Ledger forensics 07-17..08-28: **33 round-trips realized +$4.32**, gross
  traded $60.1k = **5.55× turnover / 6 weeks**, churn set 55.6% of gross;
...
4. Operator fills the confirmation slot (confirm or veto) — posted with
   timestamp on both PRs; a veto = single-key revert PR + pin re-advance.
5. Re-enable only via a rotation design that passes a WF gate + a new row.

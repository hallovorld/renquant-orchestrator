...
DECISION: complete analyst data (full coverage + multi-year history) needs a paid/entitled
          ...
          $29/mo (the #24 fetcher works immediately, collect today); (3) EOD Historical Data.
          Finnhub fetcher #25 + cron #408 (both pending re-review) accumulate a free broad-coverage
          series meanwhile.
COST:     account ~$10.6k → FMP Starter $29/mo (monthly billing; $22 = annual-prepay rate, not
          used here) ≈ 3.3% equity/yr if held a year; edge UNPROVEN (FMP prelim inside noise).
          Prefer free/entitled. If paying, default to a ONE-MONTH validation buy (verify ADI/NFLX
          unlock → pull → validate → CANCEL unless it clears a pre-registered bar; ~$29 one-time),
          NOT a recurring sub; cap at ~$29–50/mo. VALIDATE via the analyst feature's OWN pre-registered per-regime
...
NEXT:     try the MCP OAuth (FactSet or S&P) — if entitled, build an MCP-backed base-data fetcher;
          else FMP $29/mo upgrade (verify ADI/NFLX unlock) → collect today via the #24 code. Then a

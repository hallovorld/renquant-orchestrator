...
| 2 | inference-frame cache key embeds the whole `panel_scoring` block (incl. `artifact_path`) | every model swap rebuilds 145 tickers — cold rebuilds observed at ~795s and ~1201s, and a third run hit a hard 1800s timeout and ABORTED `[VERIFIED — /tmp/ptserve_e2e.log (795s), /tmp/ptprod_e2e2.log (1201s), /tmp/ptprod_e2e.log (1800.07s TimeoutError)]` | key over-specified: fields that cannot change frame content invalidate the cache | orch#589 (design) |
